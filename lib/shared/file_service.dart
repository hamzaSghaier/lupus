...
class FileService {
  ...
  static Future<File> updateBilan(BilanModel newBilan) async {
    try {
      ...
      int bilanIndex =
          listBilans.indexWhere((bilan) => bilan.id == newBilan.id);
      ...
      List<String> updatedEntries =
          listBilans.map((bilan) => jsonEncode(bilan.toJson())).toList();
    ...
    try {
      ...
      if (contents.isNotEmpty) {
        final entries = contents
            .split(";")
            .where((element) => element.isNotEmpty && element.isBlank == false);
  ...
  static Future<void> deleteRdv(RdvModel rdv) async {
    try {
      ...
      String updatedContent =
          listRdv.map((rdv) => jsonEncode(rdv.toJson())).join(';');
  ...
  static Future<RdvModel?> getLatestRDV() async {
    try {
      ...
      final futureRdvs = rdvs
          .where((rdv) => rdv.date.isAfter(DateTime.now()))
          .toList()
