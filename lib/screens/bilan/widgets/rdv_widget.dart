...
class _RdvWidgetState extends State<RdvWidget> {
    ...
    return Column(
      children: [
        ...
        const Padding(
          ...
          child: Text(
            'RDV de consultation\nموعد العيادة',
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: seedColor),
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              ...widget.rdvs.where((rdv) => rdv.type == "Consultation").map((rdv) => _buildRdvItem(rdv)),
        ...
        if (widget.rdvs.any((rdv) => rdv.type != "Consultation"))
          Container(
            ...
            child: Column(
              ...
              children: widget.rdvs.where((rdv) => rdv.type != "Consultation").map((rdv) => _buildRdvItem(rdv)).toList(),
            ),
          ),
        if (widget.hasRdv)
          ElevatedButton.icon(
            ...
            label: const Text("Autres RDVs\nمواعيد أخرى", textAlign: TextAlign.center),
  ...
  Future<void> _showDeleteConfirmation(BuildContext context, RdvModel rdv) async {
    return showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Supprimer le RDV?\nحذف الموعد؟',
              textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.normal, fontSize: 14)),
          content: const Text('Cette action ne peut pas être annulée.\nلا يمكن التراجع عن هذا الإجراء.',
              textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.normal, fontSize: 12)),
          actions: [
            TextButton(
              child: const Text(
                'Annuler | إلغاء',
      ...
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(rdv.title, style: const TextStyle(fontWeight: FontWeight.normal, fontSize: 12)),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(rdv.type, style: const TextStyle(fontWeight: FontWeight.normal, fontSize: 12)),
                Text(DateFormat('dd-MM-yyyy').format(rdv.date), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
              ],
            ),
          ),
          Row(
                ...
                onPressed: () async {
                  ...
                  if (picked != null) {
                    RdvModel updatedRdv = RdvModel(
                      ...
                      title: rdv.title,
                      type: rdv.type,
                      dateProchaineConsultation: DateFormat('dd-MM-yyyy').format(picked),
                    );
  ...
  Future<void> _showInputDialog(BuildContext context, String type) async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          type == "Consultation" ? 'Ajouter un RDV\nأضف موعدًا' : 'Ajouter un autre RDV\nأضف موعدًا آخر',
          textAlign: TextAlign.center,
        ),
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () async {
              if (titleController.text.isEmpty || rdvDateController.text.isEmpty) {
                return;
              }
            ...
            child: const Text('Ajouter | إضافة', style: TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red,
            ...
            child: const Text('Annuler | إلغاء', style: TextStyle(color: Colors.white)),
          )
        ],
      ),
    );
  ...
  Future<void> _scheduleNotification(RdvModel rdv) async {
    ...
    final notificationId = rdv.id?.abs() ?? (DateTime.now().millisecondsSinceEpoch % 100000); // Keep last 5 digits
    ...
    await AwesomeNotifications().createNotification(
      content: NotificationContent(
        ...
        title: rdv.title,
        body: "Rappel: Vous avez un rendez-vous ${rdv.type} demain à ${rdv.dateProchaineConsultation}",
  ...
  Future<void> _showEditDialog(BuildContext context, RdvModel rdv) async {
    TextEditingController titleController = TextEditingController(text: rdv.title);
    TextEditingController typeController = TextEditingController(text: rdv.type);
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Modifier | تعديل', style: TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red,
