...
class MyPainter extends CustomPainter {
  MyPainter(
    ...
    // Define color progression: red → yellow → green
    this.color = _getColorForIndex(activeIndex, unitAnimatingValue);
  }
  ...
  Color _getColorForIndex(int index, double animationProgress) {
    switch (index) {
      // case 0: // Terrible
      //   return Color.lerp(Colors.red, Colors.redAccent, animationProgress) ?? Colors.red;
      // case 1: // Bad
      //   return Color.lerp(Colors.redAccent, Colors.orange, animationProgress) ?? Colors.orange;
      // case 2: // Okay
      //   return Color.lerp(Colors.orange, Colors.yellow, animationProgress) ?? Colors.yellow;
      // case 3: // Good
      //   return Color.lerp(Colors.yellow, Colors.lightGreen, animationProgress) ?? Colors.lightGreen;
      // case 4: // Great
      //   return Color.lerp(Colors.lightGreen, Colors.green, animationProgress) ?? Colors.green;
      default:
        return Colors.grey;
    }
  }
...
class MyIndicator extends StatelessWidget {
  MyIndicator({
    ...
    // Custom color progression for indicator heads
    indicatorHeadColor = _getIndicatorColor(possition);
    indicatorYellowColor = _getIndicatorYellowColor(possition);
  }

  Color _getIndicatorColor(double position) {
    if (position < 0.2) return const Color(0xFFFF4444); // Dark Red
    if (position < 0.4) return const Color(0xFFFF7F50); // Coral
    if (position < 0.6) return const Color(0xFFFFC300); // Vivid Yellow
    if (position < 0.8) return const Color(0xFF7CFC00); // Lawn Green
    return const Color(0xFF008000); // Dark Green
  }
  ...
  late Color indicatorYellowColor;
  final double animationValue;
